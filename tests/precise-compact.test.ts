...
describe("indian system (lakh/crore)", () => {
  it("formats lakh/crore (words & abbr)", () => {
    ...
    expect(fmt.format(25_000_000, { system: "indian", style: "abbr" })).toBe(
      "2.5 Cr"
    );
  });
...
describe("eastAsia system (wan/yi)", () => {
  ...
  it("uses locale rules for joiner and order", () => {
    ...
    expect(fmt.format(10_000, { system: "eastAsia", locale: "zh-CN" })).toBe(
      "1万"
    );
    expect(
      fmt.format(100_000_000, { system: "eastAsia", locale: "zh-CN" })
    ).toBe("1亿");
    // negative
    expect(fmt.format(-20_000, { system: "eastAsia", locale: "zh-CN" })).toBe(
      "-2万"
    );
  });
});
...
describe("locale packs and rules", () => {
  ...
  it("supports resolveLabel for morphologies (ru pluralization demo)", () => {
    ...
    expect(fmt.format(2_000_000, { locale: "ru-pl", style: "abbr" })).toBe(
      "2 млн"
    );
  });
  ...
  it("unitOrder=before and NBSP joiner + finalize hook", () => {
    ...
    expect(fmt.format(2_000_000, { locale: "xx2", style: "abbr" })).toBe(
      "[MI\u00A02]"
    );
  });
...
describe("systems registry and fallbacks", () => {
  it("unknown system -> fallback raw", () => {
    expect(fmt.format(1_000_000, { system: "nope" as SystemId })).toBe(
      "1000000"
    );
  });
